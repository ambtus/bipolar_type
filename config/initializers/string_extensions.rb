...
%w{chip second third fourth words cleaned n first_word last_words last_word parenthesize wrap comma period semi colon is_mbti? to_noun s ed en ly ing an some enough many too_many too_much too_few too_little a_few plural? little few more fewer less much as_much many as_many that those is are was were them it they able un begins_with? has have do does}.each do |meth|
...
class String
  ...
  def cleaned; self.gsub('_', ' ').gsub('<wbr>', '').gsub('your self', 'yourself'); end
